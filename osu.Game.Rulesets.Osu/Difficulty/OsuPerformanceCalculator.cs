...
namespace osu.Game.Rulesets.Osu.Difficulty
{
    public class OsuPerformanceCalculator : PerformanceCalculator
    {
        ...
        protected override PerformanceAttributes CreatePerformanceAttributes(IScoreInfo score, DifficultyAttributes attributes)
        {
            ...
            if (score.Mods.Any(m => m.Acronym == "SO") && totalHits > 0)
                multiplier *= 1.0 - Math.Pow((double)osuAttributes.SpinnerCount / totalHits, 0.85);

            if (score.Mods.Any(h => h.Acronym == "RX"))
            {
        ...
        private double computeAimValue(IScoreInfo score, OsuDifficultyAttributes attributes)
        {
            ...
            if (score.Mods.Any(m => m.Acronym == "BL"))
                aimValue *= 1.3 + (totalHits * (0.0016 / (1 + 2 * effectiveMissCount)) * Math.Pow(accuracy, 16)) * (1 - 0.003 * attributes.DrainRate * attributes.DrainRate);
            else if (score.Mods.Any(m => m.Acronym == "HD" || m.Acronym == "TC"))
            {
        ...
        private double computeSpeedValue(IScoreInfo score, OsuDifficultyAttributes attributes)
        {
            if (score.Mods.Any(h => h.Acronym == "RX"))
                return 0.0;
        ...
        private double computeAccuracyValue(IScoreInfo score, OsuDifficultyAttributes attributes)
        {
            if (score.Mods.Any(h => h.Acronym == "RX"))
                return 0.0;
        ...
        private double computeFlashlightValue(IScoreInfo score, OsuDifficultyAttributes attributes)
        {
            if (score.Mods.All(h => h.Acronym != "FL"))
                return 0.0;
