...
namespace osu.Game.Screens.Ranking
{
    ...
    {
        private readonly IReadOnlyList<ScoreInfo>? leaderboardScores;

        [Resolved]
        private LeaderboardProvider leaderboardProvider { get; set; } = null!;

        public SoloResultsScreen(ScoreInfo score, IReadOnlyList<ScoreInfo>? leaderboardScores = null)
            : base(score)
        {
        ...
        protected override APIRequest? FetchScores(Action<IEnumerable<ScoreInfo>> scoresCallback)
        {
            if (leaderboardScores != null)
                setScores(scoresCallback, leaderboardScores);
            else
            {
                ...
                leaderboardProvider.GetOnlineScoresAsync(Score.BeatmapInfo!, Score.Ruleset, null, BeatmapLeaderboardScope.Global)
                                   .ContinueWith(r =>
                                   {
                                       if (r.IsFaulted)
                                       {
                                           Logger.Log($"Failed to retrieve global leaderboard: {r.Exception}");
                                           return;
                                       }

                                       var scores = r.GetResultSafely().best.ToList();
                                       setScores(scoresCallback, scores);
                                   });
            }
        ...
        private void setScores(Action<IEnumerable<ScoreInfo>> scoresCallback, IReadOnlyList<ScoreInfo> scores)
        {
